...
class TestOptimize:
    ...
    #    cnes_certificate = os.getenv("CNES_CERTIFICATE", True)
    #    package = package.replace('artifactory.cnes.fr/docker/',"")
    #    if not cnes_certificate:
    #        query = requests.get(f"https://hub.docker.com/v2/repositories/library/{package}/tags/{version}/images", verify=cnes_certificate)
    #    else:
    ...
    def test_no_temporary_artifacts_in_dockerfile(self, dockerfile_parser):
        ...
        ), f"Found patterns indicative of clean up temporary artifacts: {found_patterns}"
...
class TestMaintenance:
    ...
    def test_has_maintainer_label(self, labels_parser):
        ...
        assert (
            "org.opencontainers.image.authors" in labels_parser.keys()
        ), "LABEL instruction for authors not found"
    ...
    def test_has_version_label(self, labels_parser):
        ...
        assert (
            "org.opencontainers.image.version" in labels_parser.keys()
        ), "LABEL instruction for version not found"
        assert not labels_parser["org.opencontainers.image.authors"].isspace()

    def test_build_timestamp_label(self, labels_parser):
        """Confirm that a LABEL instruction is present in the Dockerfile specifying the build timestamp."""
        assert (
            "org.opencontainers.image.created" in labels_parser.keys()
        ), "LABEL instruction for build timestamp not found"
    ...
    def test_has_documentation_label(self, labels_parser):
        ...
        assert (
            "org.opencontainers.image.created" in labels_parser.keys()
        ), "LABEL instruction for documentation not found"
        assert not labels_parser["org.opencontainers.image.created"].isspace()
    ...
    def test_has_source_label(self, labels_parser):
        ...
        assert (
            "org.opencontainers.image.source" in labels_parser.keys()
        ), "LABEL instruction for source not found"
    ...
    def test_has_vendor_label(self, labels_parser):
        ...
        assert (
            "org.opencontainers.image.vendor" in labels_parser.keys()
        ), "LABEL instruction for vendor not found"
    ...
    def test_has_licenses_label(self, labels_parser):
        ...
        assert (
            "org.opencontainers.image.licenses" in labels_parser.keys()
        ), "LABEL instruction for licenses not found"
    ...
    def test_has_title_label(self, labels_parser):
        ...
        assert (
            "org.opencontainers.image.title" in labels_parser.keys()
        ), "LABEL instruction for title not found"
    ...
    def test_has_title_description(self, labels_parser):
        ...
        assert (
            "org.opencontainers.image.description" in labels_parser.keys()
        ), "LABEL instruction for description not found"
    ...
    def test_has_mame_label(self, labels_parser):
        ...
        assert (
            "org.opencontainers.image.ref.name" in labels_parser.keys()
        ), "LABEL instruction for name not found"
...
class TestSecurity:
    ...
    @pytest.mark.skip(reason="Certificate pb")
    def test_is_official_base_image(self, dockerfile_parser):
        ...
        assert self._is_official_image(
            base_image
        ), "No official image found in the Dockerfile"
    ...
    def test_regularly_updates_base_images_and_dependencies(self, dockerfile_parser):
        ...
        assert self._contains_update_instructions(
            dockerfile_parser.content
        ), "No update instructions found in the Dockerfile"

    def test_restrict_user_root_usage(self, dockerfile_parser):
        """Confirm that the Dockerfile restricts the use of 'USER root'."""
        assert not self._contains_user_root(
            dockerfile_parser.content
        ), "Dockerfile contains 'USER root', which is restricted"
...
class TestRunning:
    ...
    def test_contains_workdir(self, dockerfile_parser):
        ...
        dockerfile_content = dockerfile_parser.content
        assert (
            "WORKDIR /app" in dockerfile_content
        ), "Dockerfile does not contain the WORKDIR /app"
